...
/* ─── Shared classes ────────────────────────────────────────────────────────── */
const inputCls = "w-full font-mono text-xs px-2 py-1.5 rounded-sm focus:outline-none focus:border focus:border-[#1a1a1a] bg-[#3b3b3b] text-neutral-200"
const labelCls = "font-mono text-[11px] text-neutral-400"

export function Control({ name, min, max, step, val, setter, fmt }: {
  ...
  return (
    <Fragment>
      ...
      <span className="font-mono text-[11px] text-neutral-400 whitespace-nowrap">{fmt(val)}</span>
    </Fragment>
  )
}
...
export default function App() {
  ...
  /* ── ResizeObserver: update canvas size when stage div resizes ── */
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      for (const entry of entries) {
        ...
        if (w > 0 && h > 0) {
          ...
          // resize the offscreen canvas immediately so next frame is correct
  ...
  /* ── offscreen canvas ref (shared with loop) ── */
  ...
  useEffect(() => {
    ...
    // create offscreen canvas and store in ref so ResizeObserver can resize it
    const off = document.createElement("canvas")
    off.width  = canvasSizeRef.current.w
    off.height = canvasSizeRef.current.h
    offRef.current = off
    ...
    function render() {
      ...
      // sync visible canvas size every frame (cheap if unchanged)
      canvas.width  = CW
      canvas.height = CH
      off.width     = CW   // keep in sync (ResizeObserver also does this)
      off.height    = CH
      ...
      wrapText(textRef.current, scale, CW - MARGIN * 2).forEach((line, li) => {
        let lineW = 0
        for (const c of line) lineW += (glyphs[c]?.w ?? 0) * scale
        let xStart = MARGIN
        if (alignRef.current === "center") xStart = (CW - lineW) / 2
        if (alignRef.current === "right")  xStart = CW - MARGIN - lineW
        let xOff = xStart
  ...
  return (
    <div className="absolute top-0 left-0 p-6 w-full h-full bg-[#242424] flex flex-col gap-6">
      <div className="shrink-0 w-full grid grid-cols-1 md:grid-cols-4 md:h-60 gap-6">
        ...
        <div className="flex flex-col gap-4 z-10">
          ...
          <div className="flex flex-wrap gap-1.5">
            {TEXT_PRESETS.map(tp => (
              <button key={tp.label} onClick={() => setInputText(tp.text)}
          ...
          <textarea ref={textareaRef} value={inputText} onChange={e => setInputText(e.target.value)}
            ...
            className="w-full font-mono text-sm px-3 py-2 min-h-7 max-h-[70vh] rounded-sm resize-y focus:outline-none z-50 bg-[#3b3b3b] text-neutral-200" />
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-6 p-2 pt-4 rounded-md w-auto bg-[#242424] flex flex-col gap-1 items-center z-40">
              {KB_ROWS.map((row, i) => (
                <div key={i} className="flex gap-1">
                  {row.map(ch => (
                    <button key={ch} onClick={() => insertAtCursor(ch)}
                      className="px-2 h-6 border rounded-sm font-mono text-sm bg-[#1a1a1a] text-neutral-200 active:scale-95 transition-all cursor-pointer flex items-center justify-center">
                      {ch}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex gap-1 h-8">
                ...
                <button onClick={backspaceAtCursor}
                  className="w-7 flex items-center justify-center rounded-sm font-mono text-base bg-[#1a1a1a] text-neutral-200 active:scale-95 transition-all cursor-pointer">
                  ⌫
                </button>
              </div>
            </div>
        ...
        <div className="flex flex-col gap-4 col-span-1 md:col-span-2 z-10">
            ...
            <div className="flex-1 flex items-start gap-3">
              <div className="flex flex-wrap gap-2 flex-1">
                {BUILTIN_PRESETS.map((p, i) => {
                  ...
                  return (
                    <button key={`builtin-${i}`} onClick={() => applyPreset(p, `builtin-${i}`)}
            ...
            <div className="flex-1 flex flex-col gap-4">
              <div className="grid gap-x-3 gap-y-1 items-center" style={{ gridTemplateColumns: "130px 1fr auto" }}>
                {([
                  ...
                  <Control key={name} name={name} val={val} setter={setter} min={min} max={max} step={step} fmt={fmt} />
                ))}
                {([
                  ...
                  <Fragment key={name}>
                    <label className={labelCls}>{name}</label>
                    <input type="color" value={val} onChange={e => (setter as (v: string) => void)(e.target.value)} className="w-8 h-7 cursor-pointer border-0 bg-transparent p-0" />
                    <span className="font-mono text-[11px] text-neutral-400">{val}</span>
                  </Fragment>
                ))}
              ...
              {(["left", "center", "right"] as const).map(a => (
                <button key={a} onClick={() => setAlign(a)}
                  className={`px-3 py-1 border rounded-sm font-mono text-[11px] cursor-pointer transition-all
                    ${align === a ? "border-neutral-400 bg-neutral-700 text-neutral-100 opacity-100" : "border-neutral-600 bg-transparent text-neutral-500 opacity-50 hover:opacity-80"}`}>
                  {a === "left" ? "⬛▪▪" : a === "center" ? "▪⬛▪" : "▪▪⬛"}
                </button>
              ))}
            </div>
      ...
      {/* ── save modal ── */}
      {showSaveDialog && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[200]" onClick={() => setShowSaveDialog(false)}>
          <div className="bg-[#242424] border border-neutral-600 rounded shadow-xl p-5 w-72 flex flex-col gap-3" onClick={e => e.stopPropagation()}>
            <p className="font-mono text-[11px] text-neutral-400 m-0">your style set will be public.</p>
            ...
            <label className={labelCls} htmlFor="savePassword">set a password - you'll need it to delete.</label>
            ...
            <div className="flex gap-2 mt-1">
              <button onClick={handleSave} disabled={saving || !saveName.trim() || !saveAuthor.trim() || !savePassword.trim()}
                className="flex-1 py-1.5 rounded-sm font-mono text-[12px] bg-neutral-100 text-neutral-900 cursor-pointer disabled:opacity-40 transition-opacity">
                {saving ? "saving…" : "done"}
              </button>
              <button onClick={() => { setShowSaveDialog(false); setSaveName(""); setSaveAuthor(""); setSavePassword("") }}
                className="flex-1 py-1.5 rounded-sm font-mono text-[12px] border border-neutral-600 text-neutral-400 bg-transparent cursor-pointer hover:opacity-60 transition-opacity">
                cancel
              </button>
            </div>
          </div>
      ...
      {/* ── delete modal ── */}
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[200]" onClick={closeDeleteModal}>
          <div className="bg-[#242424] border border-neutral-600 rounded shadow-xl p-5 w-72 flex flex-col gap-3" onClick={e => e.stopPropagation()}>
            <p className="font-mono text-[13px] text-neutral-200 m-0">delete <strong>{deleteTarget.name}</strong>?</p>
            <p className="font-mono text-[11px] text-neutral-400 m-0">enter your deletion password.</p>
            <input type="password" placeholder="password" value={deletePassword}
              onChange={e => { setDeletePassword(e.target.value); setDeleteError("") }}
              autoFocus className={inputCls} />
            {deleteError && <p className="font-mono text-[11px] text-red-500 m-0">{deleteError}</p>}
            <div className="flex gap-2">
              <button onClick={handleDelete} disabled={deleting || !deletePassword.trim()}
                className="flex-1 py-1.5 rounded-sm font-mono text-[12px] bg-red-600 text-white border-none cursor-pointer disabled:opacity-40 transition-opacity">
      ...
      {/* ── stage: full viewport on sm, flex-grow on md+ ── */}
      <div ref={stageRef} className="relative w-screen -mx-6 md:mx-0 md:w-auto h-screen md:h-auto md:flex-1 md:min-h-0 shrink-0">
        ...
        {/* canvas sits at top-left; its pixel size is updated every frame via canvasSizeRef */}
        <div className="absolute inset-0" style={{ filter: "url(#posterize)" }}>
          <canvas id="c" width={canvasSize.w} height={canvasSize.h} className="block w-full h-full" />
        </div>
